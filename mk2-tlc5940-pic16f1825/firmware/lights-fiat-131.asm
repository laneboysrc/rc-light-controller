...
;   Rally Legends FIAT 131 Mirafiori body shell.
;
;   The hardware is based on PIC12F1840 and WS2812. No DC/DC converter is used.
;   The LEDs in the front are WS2812, and PL9823 in the back
;
...
;       LED 5       Right tail / brake / indicator 
;       LED 6       Right reversing
;       LED 7       Left tail / brake / indicator 
;       LED 8       Left reversing
;
;******************************************************************************
;
;   Author:         Werner Lane
...
;
;******************************************************************************
    TITLE       Light control logic for the Rally Legends FIAT 131
    RADIX       dec
...
#define LED_TAIL_BRAKE_INDICATOR_L light_data + (3 * 7)   
#define LED_TAIL_BRAKE_INDICATOR_R light_data + (3 * 4)
#define LED_REVERSE_L light_data + (3 * 6)   
#define LED_REVERSE_R light_data + (3 * 5)
...
#define VAL_TAIL 50
#define VAL_BRAKE 150
#define VAL_REVERSE 80
#define VAL_INDICATOR_R 40      ; Red part of orange
#define VAL_INDICATOR_G 100     ; Green part of orange
...
_output_lights_tail_brake    
    movlw   LOW LED_TAIL_BRAKE_INDICATOR_L
    movwf   FSR0L
    movlw   HIGH LED_TAIL_BRAKE_INDICATOR_L
    movwf   FSR0H
    movfw   FSR1L
    movwi   FSR0++          ; Red only ...
    movlw   LOW LED_TAIL_BRAKE_INDICATOR_R
    movwf   FSR0L
    movlw   HIGH LED_TAIL_BRAKE_INDICATOR_R
    movwf   FSR0H
    movfw   FSR1L
    movwi   FSR0++          ; Red only ...
    return
...
output_lights_reverse
    movlw   LOW LED_REVERSE_L
    movwf   FSR0L
    movlw   HIGH LED_REVERSE_L
    movwf   FSR0H
    ...
    movwi   FSR0++
    movwi   FSR0++
    movlw   LOW LED_REVERSE_R
    movwf   FSR0L
    movlw   HIGH LED_REVERSE_R
    movwf   FSR0H
