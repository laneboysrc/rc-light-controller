...
// need 8 * 4 = 32 bits, which is four uint8.
#define WS281X_BITSTREAM_SIZE (MAX_NUMBER_OF_WS281X * LEDS_PER_WS281X * 4)
static uint8_t ws281x_bitstream[WS281X_BITSTREAM_SIZE];
...
#error Clock initialization code expexts __SYSTEM_CLOCK to be set to 1200000
    ...
    // Turn on peripheral clocks for SCTimer, IOCON, SPI1, SPI0, DMA
    // (GPIO, SWM alrady enabled after reset)
    LPC_SYSCON->SYSAHBCLKCTRL |= (1 << 8) | (1 << 18) | (1 << 12) | (1 << 11) | (1 << 29);
...
void HAL_ws2811_init(uint8_t tx_pin)
{
    ...
    // Enable the DMA channel for SPI1 TX
    LPC_DMA->DMACOMMON[0].ENABLESET = (1 << DMAREQ_SPI1_TX);

    // Set the SRAM base to the DMA table
    LPC_DMA->SRAMBASE = DMA_ADDR(Chip_DMA_Table);

    // Setup DMA channel configuratiopn
    LPC_DMA->CHANNEL[DMAREQ_SPI1_TX].CFG = DMA_CFG_PERIPHREQEN | DMA_CFG_TRIGBURST_SNGL | DMA_CFG_CHPRIORITY(3);

    // Set the DMA descriptor
    Chip_DMA_Table[DMAREQ_SPI1_TX].source = DMA_ADDR(&ws281x_bitstream[WS281X_BITSTREAM_SIZE - 1]);
    Chip_DMA_Table[DMAREQ_SPI1_TX].dest = DMA_ADDR(&LPC_SPI1->TXDAT);
    Chip_DMA_Table[DMAREQ_SPI1_TX].next = DMA_ADDR(0);
    Chip_DMA_Table[DMAREQ_SPI1_TX].xfercfg =
        DMA_XFERCFG_CFGVALID |
        DMA_XFERCFG_SWTRIG |
        DMA_XFERCFG_WIDTH_8 |
        DMA_XFERCFG_XFERCOUNT(WS281X_BITSTREAM_SIZE) |
...
void HAL_ws2811_transaction(uint8_t *data, uint8_t count, bool inverted)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        ws281x_bitstream[ws_index++] = led_data[(byte >> 6) & 0x03] ^ inversion;
        ws281x_bitstream[ws_index++] = led_data[(byte >> 4) & 0x03] ^ inversion;
        ws281x_bitstream[ws_index++] = led_data[(byte >> 2) & 0x03] ^ inversion;
        ws281x_bitstream[ws_index++] = led_data[(byte >> 0) & 0x03] ^ inversion;
    }

    // Setup DMA data transfer, which starts sending data
    LPC_DMA->CHANNEL[DMAREQ_SPI1_TX].XFERCFG = Chip_DMA_Table[DMAREQ_SPI1_TX].xfercfg;
}
