...
static bool process_buffer(uint32_t *out)
{
    ...
    while (buffer_index) {
        ...
        if (checksum == excepted_checksum) {
            ...
            out[2] = *(uint16_t *)(&buffer[6 + 2*config.ibus_aux_channel_offset]) & 0xfff;
            out[3] = *(uint16_t *)(&buffer[8 + 2*config.ibus_aux_channel_offset]) & 0xfff;
            out[4] = *(uint16_t *)(&buffer[10 + 2*config.ibus_aux_channel_offset]) & 0xfff;
