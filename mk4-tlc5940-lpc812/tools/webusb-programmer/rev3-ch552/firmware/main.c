...
#define GPIO_ADDRESS_CH3 (P3_ADDR)
#define GPIO_PIN_CH3 (1)
__sbit __at(GPIO_ADDRESS_CH3 | GPIO_PIN_CH3) PIN_CH3;

#define GPIO_ADDRESS_ISP (P3_ADDR)
#define GPIO_PIN_ISP (0)
...
bool COMMAND_handler(uint8_t value)
{
    switch (value) {
        ...
        case CMD_BAUDRATE_38400:
            SBAUD1 = (uint8_t)(256 - (24000000 / 16 / 38400));
            break;

        case CMD_BAUDRATE_115200:
            SBAUD1 = (uint8_t)(256 - (24000000 / 16 / 115200));
            break;
...
{
    U1SMOD = 1;     // Enable "fast mode" (256 - fsys / 16 / baudrate)

    // IMPORTANT: do not enable UART receiver at this point, only when we
    // are ready to talk to the light controller!
    // U1REN = 1;      // Enable UART receiver

    SBAUD1 = (uint8_t)(256 - (24000000 / 16 / BAUDRATE));
    tx_idle = 1;
}
...
static void GPIO_init(void)
{
    /*
        ...
        P1.6    RX              Push/pull output / Open drain w/ pull-up
        P1.7    TX              Push/pull output

        P3.0    OUT/ISP         Push/pull output / Hi-Z
        P3.1    CH3             Push/pull output / Hi-Z
