...
**Important: Older versions are available in the branch named [*obsolete*](https://github.com/laneboysrc/rc-light-controller/tree/obsolete) in this repository.**

## Mk4 TLC5940 LPC812 and LPC832

![Mk4 light controller Rev 2](mk4-mk4p-mk4s-for-frontpage.jpg)

This iteration makes use of NXP's LPC812 micro-controller. The LPC812 contains an ARM Cortex-M0+ 32-bit core, yet costs as little as the PIC micro-controller we used in earlier variants. The firmware is now written in C. It is compiled with the Open-Source GCC compiler. The LPC812 contains a ROM-based bootloader so firmware can be flashed by using a very low cost USB-to-serial converter (less than USD 3, including shipping, on eBay).

One of the main issue for other people building earlier light controller variants was making changes to the firmware to suite their particular requirements. With the Mk4 variant we have now a **web-based configuration tool** called the **Configurator** that allows anyone to adapt the light outputs to their desired functionality without having to modify source code.
...
The light controller can also control a 2-speed or 3-speed gearbox servo, or an auxiliary servo that drives the steering wheel in the cabin of the car, or a figures head.
...
Due to improved hardware available in the LPC812 micro-controller, servo pulse reading is now auto-adaptable and should work with any RC system on the market. In addition there is also support for the [preprocessor](#preprocessor).

The preprocessor makes wiring up the light controller much simpler as only a single servo extension wire is needed to get steering, throttle and AUX channel information from the receiver to the light controller.
...
The main downside of the Mk4 design is that the LPC812 micro-controller is only available in tiny surface mount packages. It more or less requires to build the light controller on a PCB, though one may get away with an off-the-shelf TSSOP-16 adapter board.

Based on customer request, we also made the Mk4P light controller. Schematics-wise it is identical with the Mk4, but it has a pin header so that LEDs can be connected without soldering.

We also made a Mk4S variant that has 9 switching outputs instead of current-controlled outputs. The Mk4S has the same form factor as the Mk4P is great for users who want to use high-power LEDs.

There are also two variants that we haven't promoted. The Mk4S-mini is a small light controller with 3 switching outputs. It is suitable for users that require more than one switching output, but not all 9 that the Mk4S offers.

Then there is the 5ch-PP-S, a 5-channel Pre-Processor and light controller hybrid. It has the form-factor of a slightly enlarged 5-channel Pre-Processor and offers 4 switched light outputs. This variant is useful for crawlers where there are a few LEDs in the chassis (driven by the 5ch-PP-S) and the remaining LEDs in the body shell (powered by Mk4, Mk4P or Mk4S)

*And then the chip crisis of 2020/2021 happened ...*

The LPC812 micro-controller suddenly had a lead time of 52 weeks, and is nowhere to be found. Well, a few places had the tiny XSON package with 0.4 mm pin pitch. We tried building light controllers in the XSON package, but it was not practical in our primitive home workshop -- we had almost 60% failure rate.

Luckily, NXP also has a similar sister-chip named LPC832. It has slightly different features, a different pin out and comes only in TSSOP20 package. We were able to stock a few pieces of LPC832 at affordable prices and developed new circuit boards for all Mk4 variants. The firmware was adapted so that it can seamlessly work with both LPC812 and LPC832. Also the programming tools had to be adapted. As of writing of this paragraph (November 2021), also the LPC832 becomes a very scarce resource ...


## <a name="preprocessor"></a>Pre-Processor for simpler wiring

To utilize its full potential, the DIY RC Light Controller must be able to read Steering, Throttle and AUX (CH3) signals from the receiver. Such a combined signal such as CPPM is unfortunately not available on most modern surface receivers. (Update 2021: i-Bus is now available on surface receivers from Flysky!)
...
To solve this issue we added a small micro-controller into our receivers. This micro-controller directly taps into the servo outputs and generates a serial signal on a single wire that contains combined information of all channels. This way we only need to run a single servo extension wire between the chassis and the body, providing both power and data. We use this system in most of our RC cars.
...
Note that with the Mk3 WS2812 PIC12F1840 variant one could mount the light controller on the chassis and have a single 3-pole wire running to the first LED in the chain. Since we run different bodies on many of our chassis, each having different light configurations, we still use the Pre-Processor. Since the cost of the PIC12F1840 is less than USD 2 this is not an issue.

Naturally most people won't modify their receiver to add this chip, plus programming the PIC micro-controller requires investment in a proprietary programming tool.

So with the introduction of the Mk4 light controller, we also made separate circuit boards for the Pre-Processor. This enabled more users to benefit from the Pre-Processor concept.

The Pre-Processor is built upon the same NXP LCP812 (and LPC832) micro-controller as the Mk4 light controller. Actually a Pre-Processor is nothing more than a light controller without LED driver, and a special configuration in the firmware.

![The 3-channel Pre-Processor](preprocessor-closeup.jpg)

A few customers were requesting more control over lights, so we added a 5-channel Pre-Processor that offers two additional AUX channels.

![The 5-channel Pre-Processor](preprocessor-5ch-closeup.jpg)
...
This variant uses the Microchip PIC16F1825 8-bit micro-controller in a 14-pin package.
...
Due to hardware limitations, a software approach is needed to read the servo signal pulses. Various configuration options must be carefully set to ensure the light controller can inter-operate with your particular RC radio.

To program the firmware into the micro-controller, a proprietary programmer is needed. This needs to be taken into account as the cost of this programmer (~ USD 50) may be a big chunk of the budget needed to get the light controller built and running.
